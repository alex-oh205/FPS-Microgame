...
namespace Unity.FPS.AI
{
    public class DetectionModule : MonoBehaviour
    {
                ...
                if (otherActor.Affiliation != actor.Affiliation)
                {
                    ...
                    if (sqrDistance < sqrDetectionRange && sqrDistance < closestSqrDistance)
                    {
                        ...
                        if (foundValidHit)
                        {
                            ...
                            {
                                // Healers only detect when player is below max health
                                if (CheckHealerDetect(actor.gameObject, otherActor.gameObject))
                                {
                                    IsSeeingTarget = true;
        ...
        public bool CheckHealerDetect(GameObject agent, GameObject other)
        {
            // Returns false only when the agent is a healer, detected object is the player, and the player is at max health
            if (agent.CompareTag("Healer") && other.CompareTag("Player"))
            {
                Health playerHealth = other.GetComponent<Health>();
                DebugUtility.HandleErrorIfNullGetComponent<Health, DetectionModule>(playerHealth, this, gameObject);
        ...
        public virtual void OnDamaged(GameObject agent, GameObject damageSource)
        {
            // Healers only detect when player is below max health
            if (CheckHealerDetect(agent, damageSource))
            {
                TimeLastSeenTarget = Time.time;
