...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        [Tooltip("No spread on vertical axis, only in horizontal plane")]
        public bool NoSpreadY = false;

        [Tooltip("Only used when NoSpreadY is true. Projectiles will be spaced out evenly")]
        public bool SymmetricSpread = false;
        ...
        [Tooltip("Fade in at beginning of the sound and fade out at the end")] public bool UseFading = false;
        ...
        public bool IsShootingFire { get; private set; }
        ...
        void ShootShell()
        {
            ...
            nextShell.gameObject.layer = LayerMask.NameToLayer("PhysicalAmmo");
            var children = nextShell.transform.GetComponentsInChildren<Transform>(includeInactive: true);
            foreach (var child in children)
            {
                child.gameObject.layer = LayerMask.NameToLayer("PhysicalAmmo");
            }
        ...
        void UpdateContinuousShootSound()
        {
            if (UseContinuousShootSound)
            {
                if (m_WantsToShoot && (m_CurrentAmmo >= 1f || ShootType == WeaponShootType.Grapple))
                {
                    if (fadeOut != null)
                    {
                    ...
                    if (!m_ContinuousShootAudioSource.isPlaying)
                    {
                        ...
                        if (ContinuousShootStartSfx) m_ShootAudioSource.PlayOneShot(ContinuousShootStartSfx);
                        if (UseFading)
                        {
                            isFaded = false;
                            m_ContinuousShootAudioSource.volume = 0.5f;
                            fadeIn = StartCoroutine(FadeAudioSource.StartFade(m_ContinuousShootAudioSource, 0.5f, 1f));
                        }
                ...
                {
                    if (ContinuousShootEndSfx) m_ShootAudioSource.PlayOneShot(ContinuousShootEndSfx);
                    if (UseFading)
                    {
                        if (isFaded == false)
                        {
                            ...
                            fadeOut = StartCoroutine(FadeAudioSource.StartFade(m_ContinuousShootAudioSource, 0.5f, 0f));
                        }
                    } else
                    {
                        m_ContinuousShootAudioSource.Stop();
        ...
        public bool HandleShootInputs(bool inputDown, bool inputHeld, bool inputUp)
        {
            ...
            if (ShootType == WeaponShootType.Grapple)
            {
                m_WantsToShoot = IsGrappling;
            }
            switch (ShootType)
            {
        ...
        bool TryShootBeam()
        {
            if (m_CurrentAmmo >= 1f)
            {
                Vector3 beamOrigin = WeaponMuzzle.position;
                Vector3 beamDirection = WeaponMuzzle.forward;

                // Get player camera, if it exists as a child of the owner
                Camera playerCamera = null;
                Camera[] cameras = Owner.GetComponentsInChildren<Camera>();
                ...
                {
                    beamOrigin = playerCamera.transform.position;
                    beamDirection = playerCamera.transform.forward;
                }

                ProjectileBase beam = Instantiate(ProjectilePrefab, WeaponMuzzle.position, WeaponMuzzle.rotation);

                // Get closest undamageable object
                RaycastHit closestHit = new RaycastHit();
                closestHit.distance = Mathf.Infinity;
                RaycastHit[] hits = Physics.RaycastAll(beamOrigin, beamDirection, Mathf.Infinity, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Collide);
                foreach (var hit in hits)
                {
                    if (hit.distance < closestHit.distance && hit.collider.GetComponent<Damageable>() == null && !hit.collider.isTrigger && !hit.collider.GetComponent<IgnoreHitDetection>())
                    {
                        closestHit = hit;
                    }
                }
                ...
                if (closestHit.distance == Mathf.Infinity) // Handle case of no obstructions
                {
                    lineRenderer.SetPosition(1, WeaponMuzzle.InverseTransformPoint(beamOrigin + beamDirection * maxDist));
                }
                else
                {
                    ...
                    {
                        lineRenderer.SetPosition(1, WeaponMuzzle.InverseTransformPoint(closestHit.point));
                    }
                }
        ...
        void HandleShoot()
        {
            ...
            {
                Vector3 shotDirection = GetShotDirectionWithinSpread(WeaponMuzzle, bulletsPerShotFinal, i);
        ...
        public Vector3 GetShotDirectionWithinSpread(Transform shootTransform, int bulletsPerShot, int bulletIndex)
        {
            ...
            {
                float theta;
                Vector3 forward = shootTransform.forward;
                Vector3 v = Vector3.Cross(forward, Vector3.up);
                if (SymmetricSpread)
                {
                    theta =  bulletsPerShot == 1 ? 0f : (2f * bulletIndex / (bulletsPerShot - 1) - 1) * Mathf.PI * spreadAngleRatio;
                    spreadWorldDirection = forward * Mathf.Cos(theta) + v * Mathf.Sin(theta);
                }
                else
                {
                    theta = UnityEngine.Random.Range(0f, (2 * Mathf.PI));
                    Vector3 spreadVector = forward * Mathf.Cos(theta) + v * Mathf.Sin(theta);
                    spreadWorldDirection = Vector3.Slerp(shootTransform.forward, spreadVector, spreadAngleRatio);
                }
            }
            else
            {
