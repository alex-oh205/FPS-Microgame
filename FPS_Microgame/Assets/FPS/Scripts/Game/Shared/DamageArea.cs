...
namespace Unity.FPS.Game
{
    public class DamageArea : MonoBehaviour
    {
        ...
        public void InflictDamageInArea(float damage, Vector3 center, LayerMask layers,
            QueryTriggerInteraction interaction, GameObject owner, Actor ownerActor, bool stun=false, float stunDuration=3f)
        {
            ...
            foreach (var coll in affectedColliders)
            {
                ...
                if (damageable && coll.gameObject != owner && coll.transform.parent?.gameObject != owner)
                {
                    // If of the same affiliation, deal no damage
                    Actor hitActor = coll.GetComponentInParent<Actor>();
                    if (!hitActor || hitActor.Affiliation != ownerActor.Affiliation)
                    {
            ...
            foreach (Damageable uniqueDamageable in uniqueDamagedHealths.Values)
            {
                ...
                uniqueDamageable.InflictDamage(damage * DamageRatioOverDistance.Evaluate(distance / AreaOfEffectDistance), true, owner, stun, stunDuration);
            }
        }
