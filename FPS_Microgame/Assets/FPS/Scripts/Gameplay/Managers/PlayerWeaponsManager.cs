...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            SetFov(DefaultFov, AimingAnimationSpeed);
        ...
        void Update()
        {
            ...
            if (activeWeapon)
            {
                ...
                {
                    if (hit.collider.GetComponentInParent<Health>() != null && hit.collider.GetComponentInParent<PlayerCharacterController>() == null)
                    {
        ...
        void LateUpdate()
        {
            ...
            if (!isMain)
            {
                ...
                {
                    if (m_PlayerCharacterController.hookshotState != PlayerCharacterController.HookshotState.Normal)
                    {
                        GetActiveWeapon().transform.LookAt(m_PlayerCharacterController.hookshotPosition);
                        GetActiveWeapon().transform.Find("Hookshot").gameObject.SetActive(false);
                    }
                    else
                    {
                        GetActiveWeapon().transform.localRotation = Quaternion.identity;
                        GetActiveWeapon().transform.Find("Hookshot").gameObject.SetActive(true);
                    }
                }
            }
            UpdateWeaponBob();
        ...
        public void SetFov(float targetFov, float speed)
        {
        ...
        void UpdateWeaponAiming()
        {
            if (isMain)
            {
                m_PlayerCharacterController.PlayerCamera.fieldOfView = Mathf.Lerp(m_PlayerCharacterController.PlayerCamera.fieldOfView, targetFov, fovSpeed * Time.deltaTime);
            }
            WeaponCamera.fieldOfView = Mathf.Lerp(WeaponCamera.fieldOfView, targetFov, fovSpeed * Time.deltaTime) * WeaponFovMultiplier;
            if (m_WeaponSwitchState == WeaponSwitchState.Up)
            {
                ...
                {
                    activeWeapon = m_PlayerCharacterController.rightHand.GetActiveWeapon();
                }
                ...
                if (IsAiming && activeWeapon)
                {
                    ...
                    if (targetFov != activeWeapon.AimZoomRatio * DefaultFov)
                    {
                        SetFov(activeWeapon.AimZoomRatio * DefaultFov, AimingAnimationSpeed);
                    }
                }
                else
                {
        ...
        void UpdateWeaponBob()
        {
            if (Time.deltaTime > 0f)
            {
                ...
                if (m_PlayerCharacterController.IsGrounded && m_PlayerCharacterController.hookshotState == PlayerCharacterController.HookshotState.Normal)
                {
